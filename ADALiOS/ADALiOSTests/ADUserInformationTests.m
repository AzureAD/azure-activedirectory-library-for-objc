...
- (void) testCopy
{
    ...
    XCTAssertEqualObjects(userInfo.userId, copy.userId);
    XCTAssertEqualObjects(userInfo.givenName, copy.givenName);
    XCTAssertEqualObjects(userInfo.familyName, copy.familyName);
    XCTAssertEqualObjects(userInfo.identityProvider, copy.identityProvider);
...
- (void) testIdTokenNormal
{
    ...
    ADAssertNoError;
    XCTAssertEqualObjects(userInfo.userId.lowercaseString, @"boris@msopentechbv.onmicrosoft.com");
    XCTAssertEqualObjects(userInfo.familyName, @"Vidolovv");
    XCTAssertEqualObjects(userInfo.givenName, @"Boriss");
    XCTAssertEqualObjects(userInfo.rawIdToken, normalToken);
    //Test one random property:
    XCTAssertEqualObjects([userInfo.allClaims objectForKey:@"given_name"], userInfo.givenName);
}
