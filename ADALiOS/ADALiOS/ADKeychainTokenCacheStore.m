...
-(void) removeAll
{
    ...
    {
        //Get all items which are already in the cache:
        NSMutableDictionary* stored = [self getAllStoredKeys:nil];
        if (!stored)//stored could be nil too
        {
        ...
        {
            [self deleteByAttributes:attributes error:nil];//Errors are logged only
        }
    }
}
